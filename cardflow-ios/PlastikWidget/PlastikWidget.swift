...
struct CardProvider: AppIntentTimelineProvider {
    ...
    private func fetchEntry(for category: SpendCategory) async -> CardEntry {
        ...
        // Load card catalog from shared UserDefaults or bundle
        if let data = UserDefaults(suiteName: Constants.appGroupID)?.data(forKey: Constants.feedCacheKey) ?? UserDefaults.standard.data(forKey: "cachedCardData"),
           let feed = try? decoder.decode(CardDataFeed.self, from: data) {
            cards = feed.cards
        }

        // Load user cards
        if let data = UserDefaults(suiteName: Constants.appGroupID)?.data(forKey: "localUserCards") ?? UserDefaults.standard.data(forKey: "localUserCards"),
           let decoded = try? JSONDecoder().decode([UserCard].self, from: data) {
            userCards = decoded
        }

        // Find best cards for category among user's active cards
        let userCardIds = Set(userCards.filter(\.isActive).map(\.cardId))
