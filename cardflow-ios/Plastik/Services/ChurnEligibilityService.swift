...
class ChurnEligibilityService {
            ...
            for rule in rules {
                ...
                case .maxCards:
                    if let max = rule.maxCards {
                        let openCards = userCards.filter { $0.isActive && $0.closedDate == nil }
    ...
    private func checkApplicationRule(_ rule: ChurnRule, card: CreditCard, userCards: [UserCard]) -> ChurnBlocker? {
        ...
        case .productFamily:
            ...
            let hasConflicting = userCards.filter { $0.isActive && $0.closedDate == nil }.contains { conflicting.contains($0.cardId) }
            if hasConflicting {
    ...
    private func checkBonusRule(_ rule: ChurnRule, card: CreditCard, userCards: [UserCard]) -> ChurnBlocker? {
        ...
        case .bonusCooldown:
            ...
            if rule.requiresNotCurrentlyHolding == true {
                let hasCard = userCards.contains { $0.cardId == card.id && $0.isActive && $0.closedDate == nil }
                if hasCard {
